...
import { EventAttendeeSchema, EventDaySchema, EventSchema } from '@satori/domain/domain/event'
import { GroupSchema, PersonGroupSchema } from '@satori/domain/domain/group'
import { GuruSchema } from '@satori/domain/domain/guru'
import { MahakramaHistorySchema, MahakramaStepSchema } from '@satori/domain/domain/mahakrama'
...
type Tx = {
  readonly query: (
    text: string,
    params?: ReadonlyArray<string | number | boolean | null | Uint8Array | Buffer>
  ) => Effect.Effect<{ readonly rowCount: number; readonly rows: ReadonlyArray<unknown> }, DbError>
}

export class SyncRepository extends Effect.Service<SyncRepository>()('repository/SyncRepository', {
  ...
  effect: Effect.gen(function* () {
    ...
    const decodeEventAttendees = Schema.decodeUnknown(Schema.Array(EventAttendeeSchema))
    ...
    const decodePersonGroups = Schema.decodeUnknown(Schema.Array(PersonGroupSchema))
    const decodeEmpowerments = Schema.decodeUnknown(Schema.Array(EmpowermentSchema))
    const decodeGurus = Schema.decodeUnknown(Schema.Array(GuruSchema))
    const decodeMahakramaSteps = Schema.decodeUnknown(Schema.Array(MahakramaStepSchema))
    const decodeMahakramaHistory = Schema.decodeUnknown(Schema.Array(MahakramaHistorySchema))
...
`,
              [guru.id, guru.name, guru.updatedAtMs, guru.deletedAtMs, serverNowMs]
            )
            .pipe(Effect.asVoid)
        }
        case 'GuruDelete': {
...
where id = $1 and $2 >= updated_at_ms
`,
              [operation.mahakramaHistoryId, operation.deletedAtMs, serverNowMs]
            )
            .pipe(Effect.asVoid)
        }
        case 'PhotoUpsert': {
...
`,
    ...
    const getChangesSince = (tx: Tx, cursorMs: number): Effect.Effect<SyncChanges, DbError> =>
      Effect.gen(function* () {
...
`,
        ...
        const normalizedPersonRows = personsRows.map((row) => {
          ...
          const trimmedFirstName = typeof rawFirstName === 'string' ? rawFirstName.trim() : ''
          const trimmedMiddleName = typeof rawMiddleName === 'string' ? rawMiddleName.trim() : ''
          const trimmedLastName = typeof rawLastName === 'string' ? rawLastName.trim() : ''
          ...
          const middleName = trimmedMiddleName.length > 0 ? trimmedMiddleName : null
          const lastName = trimmedLastName.length > 0 ? trimmedLastName : 'Unknown'
          const email = trimmedEmail.length > 0 && isEmail(trimmedEmail) ? trimmedEmail : null
...
`,
        ...
        const eventAttendees = yield* decodeEventAttendees(eventAttendeeRows).pipe(
          Effect.mapError(
            (error) =>
              new DbError({
        ...
        const empowerments = yield* decodeEmpowerments(empowermentRows).pipe(
          Effect.mapError(
            (error) =>
              new DbError({
        ...
        const mahakramaSteps = yield* decodeMahakramaSteps(mahakramaStepRows).pipe(
          Effect.mapError(
            (error) =>
              new DbError({
        ...
        const mahakramaHistory = yield* decodeMahakramaHistory(mahakramaHistoryRows).pipe(
          Effect.mapError(
            (error) =>
              new DbError({
