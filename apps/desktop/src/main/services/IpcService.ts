...
import { IpcRoutes, makeErr, makeOk } from '@satori/ipc-contract/ipc/contract'
...
>(
  ...
  Effect.try({
    try: () => {
      ...
      ipcMain.handle(route.channel, async (event, payload: RequestEncoded) => {
        ...
        const program = Effect.gen(function* () {
          const requestExit = yield* Effect.exit(Schema.decodeUnknown(route.request)(payload))
  ...
  return {
    registerHandlers: Effect.all([
      ...
      registerHandler(IpcRoutes.eventsList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventsList(query)))
      ),
      registerHandler(IpcRoutes.eventsCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventsCreate(input)))
      ),
      registerHandler(IpcRoutes.eventsUpdate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventsUpdate(input)))
      ),
      registerHandler(IpcRoutes.eventsDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventsDelete(input)))
      ),
      registerHandler(IpcRoutes.eventDaysList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventDaysList(query)))
      ),
      registerHandler(IpcRoutes.eventDaysCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventDaysCreate(input)))
      ),
      registerHandler(IpcRoutes.eventDaysUpdate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventDaysUpdate(input)))
      ),
      registerHandler(IpcRoutes.eventDaysDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.eventDaysDelete(input)))
      ),
      ...
      registerHandler(IpcRoutes.personsList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.personsList(query)))
      ),
      registerHandler(IpcRoutes.personsCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.personsCreate(input)))
      ),
      registerHandler(IpcRoutes.personsUpdate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.personsUpdate(input)))
      ),
      registerHandler(IpcRoutes.personsDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.personsDelete(input)))
      ),

      registerHandler(IpcRoutes.attendanceList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.attendanceList(query)))
      ),
      registerHandler(IpcRoutes.attendanceCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.attendanceCreate(input)))
      ),
      registerHandler(IpcRoutes.attendanceUpdate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.attendanceUpdate(input)))
      ),
      registerHandler(IpcRoutes.attendanceDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.attendanceDelete(input)))
      ),
      registerHandler(IpcRoutes.groupsList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.groupsList(query)))
      ),
      registerHandler(IpcRoutes.groupsCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.groupsCreate(input)))
      ),
      registerHandler(IpcRoutes.groupsUpdate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.groupsUpdate(input)))
      ),
      registerHandler(IpcRoutes.groupsDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.groupsDelete(input)))
      ),
      registerHandler(IpcRoutes.personGroupsList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.personGroupsList(query)))
      ),
      ...
      registerHandler(IpcRoutes.empowermentsList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.empowermentsList(query)))
      ),
      ...
      registerHandler(IpcRoutes.gurusList, (query) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.gurusList(query)))
      ),
      registerHandler(IpcRoutes.gurusCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.gurusCreate(input)))
      ),
      registerHandler(IpcRoutes.gurusUpdate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.gurusUpdate(input)))
      ),
      registerHandler(IpcRoutes.gurusDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.gurusDelete(input)))
      ),
      ...
      registerHandler(IpcRoutes.photosCreate, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.photosCreate(input)))
      ),
      registerHandler(IpcRoutes.photosDelete, (input) =>
        authService.requireAuthenticated.pipe(Effect.zipRight(dataService.photosDelete(input)))
      ),
      ...
      registerHandler(IpcRoutes.syncStatus, () =>
        authService.requireAuthenticated.pipe(Effect.zipRight(syncService.getStatus))
      ),
      registerHandler(IpcRoutes.syncNow, () =>
        authService.requireAuthenticated.pipe(Effect.zipRight(syncService.syncNow))
      )
    ]).pipe(Effect.asVoid),
